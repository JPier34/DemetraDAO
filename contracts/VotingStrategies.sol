...
contract VotingStrategies is AccessControl, ReentrancyGuard {
    ...
    // Dynamic mapping for delegated votes
    mapping(address => mapping(ProposalCategory => address[]))
        public delegatorsList;

    mapping(address => mapping(ProposalCategory => mapping(address => uint256)))
        public delegatorIndex;

    mapping(address => mapping(ProposalCategory => mapping(address => bool)))
        public isDelegatorInList;
    ...
    constructor(
        ...
        require(
            _demetraToken != address(0),
            "VotingStrategies: token address cannot be zero"
        );
        require(
            _proposalManager != address(0),
            "VotingStrategies: proposal manager address cannot be zero"
        );
        require(
            admin != address(0),
            "VotingStrategies: admin address cannot be zero"
        );
    ...
    function vote(
        ...
        console.log(
            "VotingStrategies.vote called for proposal %s by voter %s",
            proposalId,
            voter
        );
        ...
        console.log(
            "Base voting power for voter %s: %s",
            voter,
            baseVotingPower
        );
        ...
        console.log(
            "Proposal %s uses strategy: %s",
            proposalId,
            uint256(strategy)
        );

        // Calculate effective voting power based on the strategy
        uint256 effectiveVotingPower = _calculateEffectiveVotingPower(
            voter,
            proposalId,
            strategy
        );
        console.log(
            "Effective voting power calculated: %s",
    ...
    function _calculateEffectiveVotingPower(
        ...
        uint256 baseVotingPower = demetraToken.getVotes(voter);
        console.log(
            "Calculating effective voting power for voter %s with base power %s",
            voter,
            baseVotingPower
        );
        ...
        if (strategy == ProposalManager.VotingStrategy.DIRECT) {
            console.log(
                "Using DIRECT strategy - returning base power: %s",
                baseVotingPower
            );
            ...
            uint256 categoryDelegatedPower = _calculateDynamicDelegatedVotes(
                voter,
                category
            );
            uint256 totalPower = baseVotingPower + categoryDelegatedPower;
            console.log(
                "Using LIQUID strategy - base: %s, delegated: %s, total: %s",
                baseVotingPower,
                categoryDelegatedPower,
                totalPower
            );
            return totalPower;
            ...
            console.log(
                "Using CONSENSUS strategy - returning: %s",
                consensusPower
            );
            return consensusPower;
        } else {
            console.log(
                "Unknown strategy, defaulting to base power: %s",
                baseVotingPower
            );
            return baseVotingPower;
        }
    ...
    /**
     * @dev Dynamic calculation of voting power
     */
    function _calculateDynamicDelegatedVotes(
        address delegate,
        ProposalCategory category
    ) internal view returns (uint256) {
        uint256 totalDelegatedVotes = 0;
        address[] storage delegators = delegatorsList[delegate][category];

        for (uint256 i = 0; i < delegators.length; i++) {
            address delegator = delegators[i];

            // Verify delegation is active and matches the delegate
            if (
                categoryDelegations[delegator][category].active &&
                categoryDelegations[delegator][category].delegate == delegate
            ) {
                uint256 delegatorCurrentPower = demetraToken.getVotes(
                    delegator
                );
                totalDelegatedVotes += delegatorCurrentPower;
            }
        }
    ...
    function delegateForCategory(
        ...
        require(
            delegatee != address(0),
            "VotingStrategies: cannot delegate to zero address"
        );
        require(
            delegatee != msg.sender,
            "VotingStrategies: cannot delegate to self"
        );

        address currentDelegate = categoryDelegations[msg.sender][category]
            .delegate;

        // Remove delegation if already exists
        if (
            currentDelegate != address(0) &&
            categoryDelegations[msg.sender][category].active
        ) {
        ...
        _addDelegatorToList(msg.sender, delegatee, category);
    ...
    function revokeCategoryDelegation(ProposalCategory category) external {
        ...
        require(
            currentDelegate != address(0),
            "VotingStrategies: no active delegation for category"
        );
        require(
            categoryDelegations[msg.sender][category].active,
            "VotingStrategies: delegation not active"
        );
    ...
    /**
     * @dev CORREZIONE: Aggiungi delegatore alla lista
     */
    function _addDelegatorToList(
        address delegator,
        address delegate,
        ProposalCategory category
    ) private {
        if (!isDelegatorInList[delegate][category][delegator]) {
            delegatorsList[delegate][category].push(delegator);
            delegatorIndex[delegate][category][delegator] =
                delegatorsList[delegate][category].length -
                1;
            isDelegatorInList[delegate][category][delegator] = true;
        }
    }

    /**
     * @dev CORREZIONE: Remove delegation from list
     */
    function _removeDelegatorFromList(
        address delegator,
        address delegate,
        ProposalCategory category
    ) private {
        if (isDelegatorInList[delegate][category][delegator]) {
            uint256 index = delegatorIndex[delegate][category][delegator];
            address[] storage delegators = delegatorsList[delegate][category];
            uint256 lastIndex = delegators.length - 1;

            // Substitute element with the last one (if not already the last)
            if (index != lastIndex) {
                address lastDelegator = delegators[lastIndex];
                delegators[index] = lastDelegator;
                delegatorIndex[delegate][category][lastDelegator] = index;
            }
            ...
            // Clean mapping
            delete delegatorIndex[delegate][category][delegator];
            delete isDelegatorInList[delegate][category][delegator];
        }
    }
    ...
    function _removeCategoryDelegation(
        ...
        _removeDelegatorFromList(delegator, delegatee, category);
    ...
    function getCurrentVotingPower(
        ...
        if (strategy == ProposalManager.VotingStrategy.DIRECT) {
            ...
            return
                baseVotingPower +
                _calculateDynamicDelegatedVotes(voter, category);
        } else if (strategy == ProposalManager.VotingStrategy.CONSENSUS) {
    ...
    function getPastVotingPower(
        ...
        uint256 basePastVotingPower = demetraToken.getPastVotes(
            voter,
            blockNumber
        );
    ...
    function getCategoryDelegatedVotes(
        ...
        return _calculateDynamicDelegatedVotes(delegate, category);
    }
    ...
    function getDelegatorsList(
        address delegate,
        ProposalCategory category
    ) external view returns (address[] memory) {
        return delegatorsList[delegate][category];
    }

    /**
     * @dev Suggest optimal parameters for a given voting strategy
     */
    function getSuggestedParameters(
        ProposalManager.VotingStrategy strategy
    )
        external
        view
        returns (uint256 quorum, uint256 threshold, uint256 votingPeriod)
    {
        if (strategy == ProposalManager.VotingStrategy.DIRECT) {
            return (
                strategyParams.directQuorum,
                strategyParams.directThreshold,
                strategyParams.votingPeriodDirect
            );
        } else if (strategy == ProposalManager.VotingStrategy.LIQUID) {
            return (
                strategyParams.liquidQuorum,
                strategyParams.liquidThreshold,
                strategyParams.votingPeriodLiquid
            );
        } else if (strategy == ProposalManager.VotingStrategy.CONSENSUS) {
            return (
                strategyParams.consensusQuorum,
                strategyParams.consensusThreshold,
                strategyParams.votingPeriodConsensus
            );
        } else {
            return (
                strategyParams.directQuorum,
    ...
    function updateStrategyParameters(
        ...
        require(
            directQuorum <= 5000,
            "VotingStrategies: direct quorum too high"
        );
        require(
            directThreshold >= 5000 && directThreshold <= 10000,
            "VotingStrategies: invalid direct threshold"
        );
        require(
            liquidQuorum <= 5000,
            "VotingStrategies: liquid quorum too high"
        );
        require(
            liquidThreshold >= 5000 && liquidThreshold <= 10000,
            "VotingStrategies: invalid liquid threshold"
        );
        require(
            consensusQuorum <= 5000,
            "VotingStrategies: consensus quorum too high"
        );
        require(
            consensusThreshold == 10000,
            "VotingStrategies: invalid consensus threshold"
        );
