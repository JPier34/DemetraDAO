...
contract DemetraDAO is Ownable {
    ...
    function createProposal(
        ...
        require(demetraToken.balanceOf(msg.sender) >= 100 ether, "Need 100+ tokens");
        ...
        uint256 proposalId = proposalManager.createProposal(
            msg.sender, title, description, votingPeriod,
            strategy, quorum, threshold, actions, snapshotId
        );
    ...
    function vote(uint256 proposalId, ProposalManager.VoteChoice choice) external {
        require(members[msg.sender], "Only members");
        require(!proposalManager.hasVoted(proposalId, msg.sender), "Already voted");
    ...
    function finalizeProposal(uint256 proposalId) external {
        proposalManager.finalizeProposal(proposalId, demetraToken.totalSupply());
    }
    ...
    function canVote(address voter, uint256 proposalId) external view returns (bool, string memory) {
        ...
        if (proposalManager.hasVoted(proposalId, voter)) return (false, "Already voted");
        if (demetraToken.getVotes(voter) == 0) return (false, "No voting power");
    ...
    // Treasury functions
    function withdrawFromTreasury(address payable to, uint256 amount, string memory /*reason*/) 
        external onlyOwner {
